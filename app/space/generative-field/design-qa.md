...
- The mountain sampler now spans a broad 28 by 22 world and uses deterministic ridged multifractal noise with domain warping instead of manually stacked Gaussian peaks. A noise-driven valley cuts through the range while continuous ridge fields produce a dominant left massif, an open central pass, a staggered right range, and a raised distant skyline without collapsing into two vertical walls.
- Mountain structure now follows the same particle-and-flow-line family as the other chapters. The former dense triangular wireframe was removed. Forty-eight transverse terrain contours and eleven sparse erosion paths reveal the topography while surface particles, quantized subsurface mass, slope-selected scree, and low valley mist provide separate depth and motion layers. Terrain color stays inside the selected guide palette with only a restrained neutral lift for legibility.
- Mountain visibility no longer relies on a reversed GLSL `smoothstep`, which produced undefined far-depth fading on some GPUs. Surface points now carry the silhouette ahead of the sparse contours, with a higher far-field floor that keeps the summit and distant range visible without flattening the foreground.
- Mountain wind is layered by material instead of translating the whole terrain: exposed surface points flex slightly, slope debris lifts and travels laterally, valley mist receives the longest drift, and contour lines respond only at exposed ridges. The wider camera orbit, pointer parallax, and look-target depth make those rates readable as foreground, pass, and distant-mass movement.
- Eight ridge lanes now probe the local terrain for actual height maxima. Eighteen percent of the mountain particles are resampled around those crests, while matching GPU-deformed ridge paths sit above quieter transverse contours. Ridge points receive a separate size and opacity hierarchy, and distance-aware point-size compensation keeps the summit from disappearing under perspective projection.
- Mountain particles and structural lines now retain the selected guide hue while lifting toward a cool mineral neutral. The mountain surface-line shader also replaces its reversed depth fade with a defined inverse fade, preventing ridge structure from disappearing on different WebGL drivers. Mobile uses a mountain-specific camera target so the narrow viewport reads one major slope and pass instead of an arbitrary crop.
...
- Desktop / ridged terrain rebuild: `D:\Codex\home\visualizations\2026\08\27\01a0435b-e2fa-7590-839d-90a6b0620cb0\mountain-ridged-terrain.png`
- Desktop / particle visibility and wind, first frame: `D:\Codex\home\visualizations\2026\08\27\01a0435b-e2fa-7590-839d-90a6b0620cb0\mountain-wind-depth-local.png`
- Desktop / particle visibility and wind, later frame: `D:\Codex\home\visualizations\2026\08\27\01a0435b-e2fa-7590-839d-90a6b0620cb0\mountain-wind-depth-local-later.png`
- Desktop / rebuilt ridge-particle hierarchy: `D:\Codex\home\visualizations\2026\08\27\01a0435b-e2fa-7590-839d-90a6b0620cb0\mountain-structure-final.png`
- Desktop / rebuilt ridge-particle hierarchy, later wind frame: `D:\Codex\home\visualizations\2026\08\27\01a0435b-e2fa-7590-839d-90a6b0620cb0\mountain-structure-final-later.png`
- Mobile / rebuilt ridge-particle hierarchy: `D:\Codex\home\visualizations\2026\08\27\01a0435b-e2fa-7590-839d-90a6b0620cb0\mountain-structure-mobile.png`
...
- Mobile / ridged terrain rebuild: `D:\Codex\home\visualizations\2026\08\27\01a0435b-e2fa-7590-839d-90a6b0620cb0\mountain-ridged-mobile.png`
...
The ridged-terrain mountain was re-profiled in the same headless software-rendered session as all five other chapters while moving and pressing the pointer. Its frame cost remained in the same band as society, ocean, grass, nebula, and spiral, with no runtime overlay, browser exception, or console error. The direct mountain-to-spiral jump still activated only the requested destination. Desktop and 390 by 844 captures both preserve the left massif, central pass, and right range.

## Verification

- `npm run typecheck`: passed.
- `npm run build`: passed.
- `npm run check:scope -- space-generative-field`: passed for all three changed files in the isolated publish worktree.
